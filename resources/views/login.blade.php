...
<head>
    ...
    <title>Login — DreamDay Studio</title>
    <meta name="description" content="Sign in to DreamDay Studio to curate your extraordinary event.">
...
<body class="bg-white text-[#27221e] font-sans-modern antialiased selection:bg-[#5b4b38] selection:text-white min-h-screen flex flex-col justify-between">
    ...
    <main class="flex-1 flex flex-col items-center justify-center px-4 sm:px-6 py-6 sm:py-10 w-full">
        @if(session('success_message'))
            <div class="w-full max-w-md mb-6 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-xs font-semibold text-emerald-800">
                {{ session('success_message') }}
            </div>
        @endif
        @if(session('error_message'))
        <!-- Unregistered Email Error Notice Banner -->
            ...
            <div class="flex items-start gap-3 bg-[#fdf2f2] border border-[#f8b4b4] rounded-xl p-4 shadow-xs">
                ...
                <div class="space-y-1">
                    ...
                    @if(session('unverified_email'))
                        <p class="text-xs text-[#7f1d1d]">
                            Link verifikasi akan dikirim ke <strong>{{ session('unverified_email') }}</strong>.
                        </p>
                        <form method="POST" action="{{ route('verification.send') }}" class="pt-1">
                            @csrf
                            <input type="hidden" name="email" value="{{ session('unverified_email') }}">
                            <button type="submit" class="text-xs font-bold text-[#b93838] hover:text-[#7f1d1d] underline cursor-pointer">
                                Kirim Ulang Link Verifikasi &rarr;
                            </button>
                        </form>
                    @endif
                    @if(session('unregistered_email'))
                        <p class="text-xs text-[#7f1d1d]">
                            Akun tidak ditemukan. Silakan registrasi terlebih dahulu untuk melanjutkan.
                        </p>
                        <a href="{{ route('signup') }}?email={{ urlencode(session('unregistered_email')) }}" class="inline-flex items-center gap-1.5 text-xs font-bold text-[#b93838] hover:text-[#7f1d1d] underline pt-1">
        ...
        <div class="text-center space-y-2 mb-8 sm:mb-10 max-w-md mx-auto">
            ...
            <p class="text-xs sm:text-sm text-[#685f58]">
                Sign in to curate your extraordinary event.
            </p>
        </div>
        ...
        <div class="w-full max-w-md bg-white rounded-2xl border border-[#ede7df] shadow-xl p-6 sm:p-10 transition-all">
            ...
            <form id="login-form" action="{{ route('login') }}" method="POST" class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center gap-3 border-b border-[#e2dcd4] focus-within:border-[#6b513a] pb-2 transition-colors">
                        ...
                        <input type="email" 
                               id="email" 
                               name="email" 
                               value="{{ old('email', session('unregistered_email', '')) }}"
                ...
                <div class="relative flex items-center justify-center my-4">
                    <div class="border-t border-[#ede7df] w-full"></div>
                    <span class="bg-white px-3 text-[0.65rem] font-bold tracking-widest text-[#8d8277] uppercase">ATAU</span>
                    <div class="border-t border-[#ede7df] w-full"></div>
                </div>

                <!-- Google Sign In Button -->
                <a href="{{ route('auth.google') }}"
                        class="w-full py-3 px-4 rounded-xl border border-[#ded5cb] hover:border-[#b0a597] bg-white hover:bg-[#faf8f5] text-[#27221e] font-semibold text-xs flex items-center justify-center gap-3 shadow-2xs hover:shadow-xs transition duration-200 cursor-pointer">
                ...
                <a href="{{ route('auth.apple') }}"
                        class="w-full py-3 px-4 rounded-xl border border-[#ded5cb] hover:border-[#b0a597] bg-white hover:bg-[#faf8f5] text-[#27221e] font-semibold text-xs flex items-center justify-center gap-3 shadow-2xs hover:shadow-xs transition duration-200 cursor-pointer mt-3">
                ...
                <div class="text-center pt-2">
                    <p class="text-xs text-[#685f58]">
                        ...
                        <a href="{{ route('signup') }}" class="font-semibold text-[#6b513a] hover:underline">Sign up</a>
                    </p>
                </div>
            </form>
        ...
        <div class="bg-white rounded-3xl border border-[#ede7df] shadow-2xl max-w-md w-full p-6 sm:p-8 space-y-5 animate-in fade-in zoom-in duration-200">
            <div class="flex items-center justify-between pb-3 border-b border-[#f2ece5]">
                <div class="flex items-center gap-2.5">
                    ...
                    <span class="font-bold text-sm text-[#27221e]">Login dengan Akun Google</span>
                </div>
            ...
            <div class="pt-3 border-t border-[#f2ece5] space-y-3">
                ...
                <form action="{{ route('auth.google') }}" method="POST" class="space-y-3">
                    ...
                    <button type="submit" class="w-full py-2.5 rounded-xl bg-[#5b4b38] hover:bg-[#483b2c] text-white font-bold text-xs transition duration-200 cursor-pointer shadow-xs">
                        Masuk / Daftar Otomatis dengan Akun Ini →
                    </button>
                </form>
            </div>
